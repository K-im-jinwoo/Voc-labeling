    ...
    try:
        ...
        if request.method == "GET":
            if "category_product" in request.GET:
                ...
                context["product_names"] = (
                    main_models.Category.objects.all()
            ...
            if request.POST["export"] == ".xlsx export":
                ...
                categorys = main_models.Category.objects.filter(
                    category_product=request.POST["product"]
                ).values_list("category_middle", flat=True)
                with BytesIO() as b:
                    ...
                    for category in categorys:
                        ...
                        for emotion in ["positive", "negative", "neutral"]:
                            temp_keyword = list(
                                all_keywords.filter(
                                    category_id__category_middle=category,
                            ...
                            product_group = [request.POST["product"]] * max(
                                counts.values()
                            )
