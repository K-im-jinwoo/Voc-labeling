...
from mainapp.models import Review, Category, WebStatus
...
def assignment(request):
    ...
    if not WebStatus.objects.filter(status_name='auto_assignment_value').exists():
        make_webStatus()

    #### ---- 화면에 보낼 데이터 ---- ####
    context = dict()
    users = User.objects.all()
    categorys = Category.objects.all().values('category_product').distinct()
    context['users'] = users
    context['category_products'] = categorys
    context['auto_assignment_value'] = WebStatus.objects.get(status_name='auto_assignment_value').status_value
    context['auto_assignment_status'] = WebStatus.objects.get(status_name='auto_assignment_status').status_value
